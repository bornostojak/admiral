...
export function ProcessCommand(args: string[]) {
    ...
    try {
        ...
        fs.writeFileSync(path.join(projectPath, "production.env"), "", { flag: 'w+' })
        fs.writeFileSync(path.join(projectPath, "staging.env"), "", { flag: 'w+' })
        fs.writeFileSync(path.join(projectPath, "theater.env"), "", { flag: 'w+' })
    } catch(err) {
